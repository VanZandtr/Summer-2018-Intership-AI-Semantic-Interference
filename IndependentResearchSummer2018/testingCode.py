...
def continuous_naming(subject_directory, norms_obj, blocks, repeat_num=4, verbose=False):
    ...
    time_record = numpy.zeros((repeat_num * len(subject_file_list),len(blocks) * len(blocks[0])))
    ...
    for i in range(0,len(groups)): #loop through groups
        group = groups[i]
        partSize = len(subject_file_list)//len(groups)
        for subj_id in range(i*partSize, i*partSize+partSize):
